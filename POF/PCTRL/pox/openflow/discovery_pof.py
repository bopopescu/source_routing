...
class LLDPSender(object):
    ...
    def _handle_openflow_PortStatus(self, event):
        ...
        if port_name.find("0") == -1 and port_name.find("v") == -1 and port_name.find("r") == -1 and port_name.find(
                "s") == -1 and port_name.find("O") == -1:
            time.sleep(0.5)
    ...
    def del_switch(self, dpid, set_timer=True):
        self._this_cycle = [p for p in self._this_cycle if
                            p.dpid != dpid]  # cc:remove p from this_cycle if p.dpid==dpid
...
class Discovery(EventMixin):
    ...
    def install_flow(self, con_or_dpid, priority=None):
        ...
        core.PofManager.add_flow_table(con.dpid, 'FirstEntryTable', of.OF_MM_TABLE, 32,
                                       [core.PofManager.get_field("DMAC")[0], core.PofManager.get_field("Eth_Type")[0]])
