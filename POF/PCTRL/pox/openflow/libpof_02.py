...
OFP_MAX_BUCKET_NUMBER_PER_GROUP = 6 # Define the max bucket number in one group. {@value}.  add by shengrulee
OFP_MAX_ACTION_NUMBER_PER_BUCKET = 6 # Define the max action number in on group bucket. {@value}. add by shengrulee
...
class ofp_bucket(ofp_base):
    _MIN_LENGTH = 16
    _MAX_LENGTH = _MIN_LENGTH + OFP_MAX_ACTION_NUMBER_PER_BUCKET * ofp_action_base._MAX_LENGTH  # 16+6*48=304

    def __init__(self, **kw):
        self.action_num = 0  # 2 bytes,
        self.weight = 0  # 2 bytes, weitht of bucket. Only defined for select groups.
        self.watch_slot_id = 0  # 2 bytes, Only required for fast failover groups.
        self.watch_port = 0  # 1 byte, Only required for fast failover groups.
        # 1 byte padding
        self.watch_group = 0  # 4 bytes, Only required for fast failover groups.
        # 4 bytes padding
        self.action_list = []  # actions in it

    # def __init__(self, **kw):
    #     self.action_num = 0     # 1 bytes.
    #     self.watch_port_type = 0 # 1 bytes, 0:immediate value, 1:packet/metadata field.
    #     self.weight = 0         # 2 bytes, weitht of bucket. Only defined for select groups.
    #     self.watch_group = 0  # 4 bytes, Only required for fast failover groups.
    #     self.watch_port_id = 0 # 4 bytes, 16-31 bits are slot id, and the least significant,\
    #                                     # 16(0-15) bits are port id.
    #     self.watch_port_id_field = None # ofp_match20
    #     self.action_list = []   # actions in it

    def pack(self):
        assert self._assert()
        packed = b''
        packed += struct.pack("!HHHB", self.action_num, self.weight, self.watch_slot_id, self.watch_port)
        packed += _PAD
        packed += struct.pack("!L", self.watch_group)
        packed += _PAD4

        for a in self.action_list:
            packed += a.pack()
            packed += _PAD * (ofp_action_base._MAX_LENGTH - len(a))

        packed += _PAD * (OFP_MAX_ACTION_NUMBER_PER_BUCKET - self.action_num) * ofp_action_base._MAX_LENGTH
    ...
    #     assert self._assert()
    #     packed = b''
    #     packed += struct.pack("!BBHL", self.action_num, self.watch_port_type, self.weight, self.watch_group)
    #
    #     if self.watch_port_type == 0:
    #         packed += struct.pack("L", self.watch_port_id)
    #         packed += _PAD4
    #     elif self.watch_port_type == 1 and self.watch_port_id_field != None:
    #         packed += self.watch_port_id_field.pack() # ofp_match20.pack
    #     else:
    ...
    def unpack(self, raw, offset=0):
        _offset = offset
        offset, (self.action_num, self.watch_port_type, self.weight, self.watch_group) = \
            _unpack("!BBHL", raw, _offset)

        if self.watch_port_type == 0:
            offset, self.watch_port_id = _unpack("!L", raw, offset)
            offset = _skip(raw, offset, 4)
        elif self.watch_port_type == 1 and self.watch_port_id_field != None:
            offset, self.watch_port_id_field = ofp_match20.unpack(raw, offset)
        else:
            offset = _skip(raw, offset, ofp_match20._MIN_LENGTH)

        offset, self.action_list = _unpack_actions(raw, \
                                                   OFP_MAX_ACTION_NUMBER_PER_GROUP * ofp_action_base._MAX_LENGTH, \
                                                   offset)
        assert offset - _offset == len(self)
        return offset

    def __len__(self):
        return ofp_bucket._MAX_LENGTH

    def show(self, prefix=''):
        outstr = ''
        outstr += prefix + 'action_num:    ' + str(self.action_num) + '\n'
        outstr += prefix + 'weight:        ' + str(self.weight) + '\n'
        outstr += prefix + 'watch_slot_id: ' + str(self.watch_slot_id) + '\n'
...
class ofp_group_mod (ofp_header):
    '''
    modified by shengrulee
    '''
    _MIN_LENGTH = ofp_header._MIN_LENGTH + 16  # 24 bytes
    _MAX_LENGTH = _MIN_LENGTH + OFP_MAX_BUCKET_NUMBER_PER_GROUP * ofp_bucket._MAX_LENGTH # 1240 bytes
    ...
    def __init__ (self, **kw):
        ...
        self.bucket_num = 0     # 1 byte
                                # 1 byte padding
        self.group_id = 0       # 4 bytes
        self.counter_id = 0     # 4 bytes
        self.slot_id = 0        # 2 bytes
                                # 2 bytes padding
        self.bucket_list = []   # ofp_bucket
    ...
    def pack (self):
        ...
        packed = b""
        packed += ofp_header.pack(self)
        packed += struct.pack("!BBB", self.command, self.group_type, self.bucket_num)
        packed += _PAD
        packed += struct.pack("!LLH", self.group_id, self.counter_id, self.slot_id)
        packed += _PAD2

        for b in self.bucket_list:
            packed += b.pack()
            packed += _PAD * (ofp_bucket._MAX_LENGTH - len(b))

        packed += _PAD * (OFP_MAX_BUCKET_NUMBER_PER_GROUP - len(self.bucket_list)) * ofp_bucket._MAX_LENGTH

        return packed
    
    def unpack (self, raw, offset=0):  # FIXME:
        offset, length = self._unpack_header(raw, offset)
        offset, (self.command, self.group_type, self.bucket_num) = \
            _unpack("!BBB", raw, offset)
        offset = _skip(raw, offset, 1)
        offset, (self.group_id, self.counter_id, self.slot_id) = _unpack("!LLH", raw, offset)
        offset = _skip(raw, offset, 2)

        for _ in xrange(0, OFP_MAX_BUCKET_NUMBER_PER_GROUP):
            b = ofp_bucket()
            offset += b.unpack(raw, offset)
            self.bucket_list.append(b)

        offset = _skip(raw, offset , (OFP_MAX_BUCKET_NUMBER_PER_GROUP - len(self.bucket_list)) * ofp_action_base._MAX_LENGTH)
    ...
    def __eq__ (self, other):
        ...
        if self.bucket_num != other.bucket_num: return False
        if self.group_id != other.group_id: return False
        if self.counter_id != other.counter_id: return False
        if self.slot_id != other.slot_id: return False
        if self.bucket_list != other.bucket_list: return False
        return True
    ...
    def show (self, prefix=''):
        ...
        outstr += prefix + 'bucket_num: ' + str(self.bucket_num) + '\n'
        outstr += prefix + 'group_id:   ' + str(self.group_id) + '\n'
        outstr += prefix + 'counter_id: ' + str(self.counter_id) + '\n'
        outstr += prefix + 'slot_id:    ' + str(self.slot_id) + '\n'
        outstr += prefix + 'bucket_list: \n'
        for obj in self.bucket_list:
            outstr += obj.show(prefix + '  ')
        return outstr
