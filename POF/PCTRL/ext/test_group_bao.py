...
class Test(EventMixin):
    ...
    def install_table_mod(self, device_id):
        core.PofManager.add_flow_table(device_id, \
                                       table_name='FirstEntryTable', \
                                       table_type=pof.OF_MM_TABLE, \
                                       table_size=32, \
                                       match_field_list = [core.PofManager.get_field("Eth_Type")[0]])

        table_id = core.PofManager.get_flow_table_id(device_id, \
                                                     table_name='FirstEntryTable')  # 0
        ...
        core.PofManager.add_flow_entry(device_id, \
                                       global_table_id = table_id,\
                                       matchx_list = [temp_matchx], \
                                       instruction_list = [temp_ins])


    def install_group_mod(self, device_id):
        action1 = pof.ofp_action_output(port_id= 0x1)
        ...
        bucket1.watch_port = 0x1
        bucket1.watch_group = 8

        action2 = pof.ofp_action_output(port_id= 0x2)
        ...
        group_msg2 = pof.ofp_group_mod()
        group_msg2.command = pof.OFPGC_ADD
        group_msg2.group_type = pof.OFPGT_FF
        group_msg2.group_id = 2
        group_msg2.slot_id = 0
        group_msg2.counter_id = 1
        group_msg2.bucket_list.append(bucket1)
        group_msg2.bucket_list.append(bucket2)
        group_msg2.bucket_num = len(group_msg2.bucket_list)
        core.PofManager.write_of(device_id, group_msg2)


    def _handle_ConnectionUp(self, event):
        self.enable_port(event.dpid)

        if event.dpid == 1:

            self.install_group_mod(event.dpid)

            self.install_table_mod(event.dpid)
