...
def _install_write_port_to_metadata_entry_protect(dpid):
    table_id = core.PofManager.get_flow_table_id(dpid, 'WritePortToMetadataTable')
    ins_write_port_to_metadata = core.PofManager.new_ins_write_metadata_from_packet( \
        metadata_offset=32 + MAX_BIT_OUTPUT_PORT - PORT_FIELD_LEN, \
        write_length=PORT_FIELD_LEN, \
...
def _install_ttl_match_entry(dpid, match_value, match_mask, prior= 800):
    ...
    # next_table_id = core.PofManager.get_flow_table_id(dpid, 'CheckPortNumTable')
    ins_go_to_table = core.PofManager.new_ins_goto_direct_table(next_table_id, 0, 0, 0, None)
    ins_list.append(ins_go_to_table)
    core.PofManager.add_flow_entry(dpid, table_id, [temp_matchx], ins_list, prior, False)

def _install_ttl_match_entry_protect(dpid, match_value, match_mask, prior= 800):
    table_id = core.PofManager.get_flow_table_id(dpid, 'CheckTTL')
    match_ttl = core.PofManager.get_field('TTL')[0]
    temp_matchx = core.PofManager.new_matchx(match_ttl, match_value, match_mask)
    ins_list = []
...
class SourceRouting(EventMixin):
    ...
    def _handle_PacketIn(self,event):
        ...
        a = packet.find('ipv4')
        if event.dpid==1:
            if a and a.dstip=="10.0.0.2":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000002', mask='FFFFFFFF')
                encap_sr_header(1,output_port=2,matchx=matchx,port_list=[2,2,2,2])
            if a and a.dstip=="10.0.0.3":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000003',mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2, 3])
            if a and a.dstip == "10.0.0.4":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000004',mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2, 4])
            if a and a.dstip=="10.0.0.5":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000005',mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2, 5])
            if a and a.dstip == "10.0.0.6":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000006',mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2,6])
            if a and a.dstip == "10.0.0.7":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000007',mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2,7])
            if a and a.dstip == "10.0.0.8":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000008',mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2,8])
            if a and a.dstip == "10.0.0.9":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a000009', mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2,9])
            if a and a.dstip == "10.0.0.10":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a00000a', mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2, 'a0'])
            if a and a.dstip == "10.0.0.11":
                matchx = core.PofManager.new_matchx(field=core.PofManager.get_field('DIP')[0], value='0a00000b', mask='FFFFFFFF')
                encap_sr_header(1, output_port=2, matchx=matchx, port_list=[2, 2, 2, 'b0'])
