...
def encap_sr_header(dpid, matchx_list, output_port, port_list):
    ...
    ######
    return encap_sr_header_multicast(dpid, matchx_list, [output_port], [port_list])
...
def encap_sr_header_multicast(dpid, matchx_list, output_port_list, port_list_group):
    ...
    for port, port_list in zip(output_port_list, port_list_group):
        ...
        metadata_start_pos_each_header.append((metadata_offset_current, metadata_offset - metadata_offset_current)) # element:(offset, length)
...
def _assemble_sr_header_in_metadata(port_list, metadata_offset = 32):
    ...
    for k in range(int(math.ceil(float(ttl) / port_num_each_ins))):
        ...
        for i in range(n):
            port_each_ins_str = port_each_ins_str + str(port_list[k * port_num_each_ins + i])
...
def encap_sr_header_w_pktout(dpid, matchx_list, output_port, port_list, event, out_dpid):
    ...
    return encap_sr_header(dpid, matchx_list, output_port, port_list)
...
def encap_sr_header_by_dip(dpid, dip, dip_mask, output_port, port_list):
    ...
    return encap_sr_header(dpid, [dip_matchx], output_port, port_list)
...
def encap_sr_header_by_dip_dport_w_pktout(dpid, dip, dip_mask, dport, dport_mask, output_port, port_list, event, out_dpid):
    dip_match = core.PofManager.get_field('DIP')[0]
    dip_matchx = core.PofManager.new_matchx(dip_match, dip, dip_mask)

    dport_match = core.PofManager.get_field('DPORT')[0]
    dport_matchx = core.PofManager.new_matchx(dport_match, dport, dport_mask)

    return encap_sr_header_w_pktout(dpid, [dip_matchx, dport_matchx], output_port, port_list, event, out_dpid)

def encap_sr_header_by_dip_sport_w_pktout(dpid, dip, dip_mask, sport, sport_mask, output_port, port_list, event, out_dpid):
    dip_match = core.PofManager.get_field('DIP')[0]
    dip_matchx = core.PofManager.new_matchx(dip_match, dip, dip_mask)
    ...
    return encap_sr_header_w_pktout(dpid, [dip_matchx, sport_matchx], output_port, port_list, event, out_dpid)

def encap_sr_header_by_dip_sport_drop(dpid, dip, dip_mask, sport, sport_mask):
    table_id = core.PofManager.get_flow_table_id(dpid, 'SRHeaderEncapTable')
...
class SourceRouting(EventMixin):
    ...
    def _handle_ConnectionUp(self, event):
        ...
        #                                [core.PofManager.get_field('DIP')[0], core.PofManager.get_field('DPORT')[0]])
        # core.PofManager.add_flow_table(event.dpid, 'SRHeaderEncapTable', pof.OF_MM_TABLE, 3000, \
        #                                [core.PofManager.get_field('DIP')[0], core.PofManager.get_field('SPORT')[0]])
        core.PofManager.add_flow_table(event.dpid, 'SRHeaderEncapTable', pof.OF_MM_TABLE, 3000, \
                                       [core.PofManager.get_field('DIP')[0]])
        core.PofManager.add_flow_table(event.dpid, 'ARPTargetIPTable', pof.OF_MM_TABLE, 1000, \
